...
import { 
  ...
  CheckCircle2, ChevronDown, RotateCcw, Volume2, Square,
  Download, FileDown, History, Lock, Sparkles 
} from "lucide-react";
...
interface DocxState { zip: JSZip; documentXml: string; fileName: string; }
interface RunSlot { text: string; paraIndex: number; }
...
const extractRuns = (xml: string): { slots: RunSlot[]; plain: string } => {
  ...
  return { slots, plain: paragraphsText.join(PARA_BREAK) };
};
...
const redistribute = (slots: RunSlot[], corrected: string): string[] => {
  ...
  slots.forEach((s, i) => { const arr = byPara.get(s.paraIndex) ?? []; arr.push(i); byPara.set(s.paraIndex, arr); });
  ...
  for (const [pIdx, indices] of byPara) {
    ...
    if (indices.length === 1) { result[indices[0]] = correctedPara; continue; }
    if (totalOriginal === 0) { result[indices[indices.length - 1]] = correctedPara; continue; }
    let cursor = 0;
    const correctedLen = correctedPara.length;
    for (let k = 0; k < indices.length; k++) {
      const isLast = k === indices.length - 1;
      if (isLast) { result[indices[k]] = correctedPara.slice(cursor); }
      else {
        ...
        for (let d = 0; d <= window; d++) {
          if (target + d <= correctedLen && /\s/.test(correctedPara[target + d - 1] ?? "")) { snap = target + d; break; }
          if (target - d > cursor && /\s/.test(correctedPara[target - d - 1] ?? "")) { snap = target - d; break; }
        }
...
const Editor = () => {
  ...
  const handleClear = () => {
    setPdfLoaded(false); setInputText(""); setOutputText(""); setDocx(null);
  ...
  const handleListen = () => {
    if (speaking) { window.speechSynthesis.cancel(); setSpeaking(false); }
      ...
      const bgVoices = voices.filter(v => v.lang.includes('bg'));
      let bestVoice = bgVoices.find(v => v.name.includes('Google') || v.name.includes('Premium'));
      if (!bestVoice && bgVoices.length > 0) bestVoice = bgVoices[0];
      if (bestVoice) utt.voice = bestVoice;
      utt.rate = 0.9; utt.pitch = 0.85; utt.volume = 1.0;
  ...
  const handlePdfChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e: any) => {
      try {
        ...
        if (isScanned) {
          toast.error("❌ This PDF is scanned or image-based and cannot be read.", { duration: 6000 });
          return;
        }
        setTone("Standard"); setPdfLoaded(true); toast.success("PDF loaded!");
      } catch (err: any) {
  ...
  const handleDownloadWord = async () => {
    if (!correctedFileBase64) { toast.error("Document is not ready yet."); return; }
    try {
      ...
      if (!isPro) { localStorage.setItem("freeWordUsed", "true"); setFreeWordUsed(true); }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      doc.setFont("NotoSans"); doc.setFontSize(11);
      const pageWidth = doc.internal.pageSize.getWidth();
      const margin = 20; const maxWidth = pageWidth - margin * 2;
      const pageHeight = doc.internal.pageSize.getHeight();
      const lineHeight = 7; let y = 25;
      for (const line of outputText.split("\n")) {
        ...
        for (const wLine of wrapped) {
          ...
          doc.text(wLine, margin, y); y += lineHeight;
        }
      }
      const safeName = fileName ? fileName.replace(/\.[^.]+$/, "") : "corrected_document";
      doc.save(`${safeName}.pdf`);
      toast.success("PDF downloaded!");
    } catch (err: any) { toast.error("PDF export failed: " + err.message); }
  };
  ...
  return (
    <section id="editor" className="container py-16 md:py-24">
      ...
      <div className="relative mx-auto max-w-4xl rounded-2xl border border-border bg-gradient-card p-4 md:p-6 shadow-card-premium backdrop-blur">
        ...
        <div className="mb-5 flex items-center gap-2 w-full">
          <button
            ...
            onClick={async () => {
              if (isPro) {
                const { data: historyData } = await supabase.from('history' as any).select('*').eq('user_id', user!.id).order('created_at', { ascending: false }).limit(20);
        ...
        <div>
          ...
          <input ref={docxInputRef} type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={handleFileChange} className="hidden" />
          ...
          <div className="mt-2 flex items-center justify-between gap-3 text-xs">
            ...
            <div className="inline-flex items-center gap-3 text-muted-foreground tabular-nums shrink-0">
              ...
              <span><span className="text-foreground font-semibold">{inputText.length}</span> chars</span>
            </div>
          </div>
        </div>
          ...
          <div className="mt-3 flex flex-wrap items-center justify-end gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44 bg-popover border-border">
                ...
                <DropdownMenuItem onClick={(e) => { if (!isPro) { e.preventDefault(); setPdfModalOpen(true); return; } handleDownloadPdf(); }} className="cursor-pointer">
            ...
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              {corrections.length > 0 && (
                <span className="inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary">
                  <CheckCircle2 className="h-3 w-3" /> {corrections.length}
                </span>
              )}
            </div>
            {corrections.length === 0 ? (
              <div className="flex items-center gap-3 rounded-lg border border-dashed border-border bg-secondary/30 p-4 text-sm text-muted-foreground">
                <CheckCircle2 className="h-5 w-5 shrink-0 text-primary" /> Your text looks great — no corrections needed.
              </div>
            ) : (
              <ul className="grid gap-3 sm:grid-cols-2">
                {corrections.map((c, i) => (
                  <li key={`${i}-${c.original}`} className="group rounded-xl border border-border bg-card/80 p-3.5 shadow-sm transition-smooth hover:border-primary/40 hover:shadow-emerald">
                    <div className="flex flex-wrap items-center gap-2 text-sm">
                      <span className="rounded-md bg-destructive/10 px-2 py-0.5 font-medium text-destructive line-through decoration-destructive/60">{c.original}</span>
                      <span className="text-muted-foreground">→</span>
                      <span className="rounded-md bg-primary/10 px-2 py-0.5 font-semibold text-primary">{c.corrected}</span>
                    </div>
                    <p className="mt-2 flex items-center gap-1.5 text-[11px] uppercase tracking-wider text-muted-foreground">
                      <Sparkles className="h-3 w-3 text-primary/70" /> {c.reason}
                    </p>
                  </li>
                ))}
              </ul>
        ...
        <div className="flex flex-col items-center gap-2 mt-5 w-full">
          <Button variant="outline" size="lg" disabled={!outputText}
      ...
      <HistoryDrawer open={historyDrawerOpen} onOpenChange={setHistoryDrawerOpen} history={history}
        onSelect={(item) => { setInputText(item.original_text); setOutputText(item.fixed_text); setTone(item.tone || "Standard"); }} />
      ...
      <Dialog open={wordModalOpen} onOpenChange={setWordModalOpen}>
        <DialogContent className="sm:max-w-md border-primary/30 bg-gradient-card shadow-emerald backdrop-blur">
          <DialogHeader>
            <div className="mx-auto mb-2 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-emerald shadow-emerald"><Upload className="h-7 w-7 text-primary-foreground" /></div>
            <DialogTitle className="text-center font-display text-2xl">Word Upload is a <span className="text-gradient-emerald">Pro Feature</span></DialogTitle>
            <DialogDescription className="text-center">You've used your 1 free Word document translation! Upgrade to Pro to unlock unlimited Word & PDF processing with perfect formatting preserved.</DialogDescription>
          </DialogHeader>
          <DialogFooter className="sm:flex-col sm:space-x-0 gap-2 mt-4">
            <Button variant="emerald" size="lg" className="w-full" onClick={() => { setWordModalOpen(false); goToPricing(); }}><Sparkles className="h-4 w-4 mr-2" /> Get Pro Now</Button>
            <button type="button" onClick={() => setWordModalOpen(false)} className="text-xs text-muted-foreground hover:text-foreground transition-smooth mt-2">Maybe later</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={creditModalOpen} onOpenChange={setCreditModalOpen}>
        <DialogContent className="sm:max-w-md border-primary/30 bg-gradient-card shadow-emerald backdrop-blur">
          <DialogHeader>
            <div className="mx-auto mb-2 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-emerald shadow-emerald"><FileDown className="h-7 w-7 text-primary-foreground" /></div>
            <DialogTitle className="text-center font-display text-2xl">PDF Limit <span className="text-gradient-emerald">Reached</span></DialogTitle>
            <DialogDescription className="text-center">
              You've used all 15 monthly PDF credits. Get <span className="font-semibold text-foreground">20 extra PDF credits</span> for only <span className="font-semibold text-foreground">€3.99</span> — one-time payment.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="sm:flex-col sm:space-x-0 gap-2">
            <Button variant="emerald" size="lg" className="w-full" disabled={buyingCredits} onClick={() => handleBuyCredits('extra_pdf_credits')}>
              ...
              : <><Sparkles className="h-4 w-4 mr-2" />Buy 20 PDF Credits — €3.99</>}
            </Button>
            <button type="button" onClick={() => setCreditModalOpen(false)} className="text-xs text-muted-foreground hover:text-foreground transition-smooth">Maybe later</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Text Limit modal */}
      <Dialog open={textCreditModalOpen} onOpenChange={setTextCreditModalOpen}>
        <DialogContent className="sm:max-w-md border-primary/30 bg-gradient-card shadow-emerald backdrop-blur">
          <DialogHeader>
          ...
          <DialogFooter className="sm:flex-col sm:space-x-0 gap-2">
            <Button variant="emerald" size="lg" className="w-full" disabled={buyingCredits} onClick={() => handleBuyCredits('extra_text_credits')}>
              {buyingCredits ? <span className="flex items-center gap-2"><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Redirecting...</span>
